...
# grant_fy_country <- readxl::read_excel("grant_fy_country.xlsx")

new_names <- c("Grant", "FY", "Country", "Connected_Actual", "Improved_Actual", "Transformed_Actual")

grant_fy_country <- grant_fy_country %>% 
  set_names(new_names)

reactable(grant_fy_country, groupBy = c("Grant", "Country"))
...
target <- processed_target %>% 
  select(benchmark_grant_friendly_name, target_number_reached:total_target_number_transformed, report_fiscal,benchmark_beneficiary_country_location )
...
```{r}
fy_grant<- grant_fy_country %>% group_by(FY, Grant) %>% 
            summarise(across(where(is.numeric), sum, na.rm = T))
...
## S2S Grant FY
```{r}
grant_fy <- grant_fy_country %>% group_by( Grant, FY) %>% 
            summarise(across(where(is.numeric), sum, na.rm = T))

reactable(grant_fy, groupBy = c("Grant", "FY"), 
           columns = list(
...
```{r}
gg_grants <- readxl::read_excel("live_grants.xlsx", sheet = "all_grants") %>% 
  janitor::clean_names() %>% 
  mutate(start_date = as_date(start_date), 
         end_date = as_date(end_date)) %>% 
  select(friendly_name, funding_foundation, grant_status, start_date, end_date, target_cash_amount_amount,  actual_cash_amount_amount, impact_location_country,impact_location_market_unit,sponsoring_location_country, 
...
payment <-
  ...
  filter(
    ...
    funding_foundation != "-",
    !grant_status %in% c("New", "In Progress", "Closed")
  ) %>%
  mutate(
